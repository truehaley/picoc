...
#define TOK_DEF(name, prefix, postfix, infix, repr, string) { prefix, postfix, infix, repr },
static const struct OpPrecedence OperatorPrecedence[] = {
    #include "token-defs.h"
};


#ifdef DEBUG_EXPRESSIONS
static void ExpressionStackShow(Picoc *picoc, struct ExpressionStack *StackTop);
#endif
...
static void ExpressionStackPushValueNode(ParseState *Parser, struct ExpressionStack **StackTop, Value *ValueLoc);
static Value *ExpressionStackPushValueByType(ParseState *Parser, struct ExpressionStack **StackTop, ValueType *PushType);
static void ExpressionStackPushValue(ParseState *Parser, struct ExpressionStack **StackTop, Value *PushValue);
static void ExpressionStackPushLValue(ParseState *Parser, struct ExpressionStack **StackTop, Value *PushValue, int Offset);
static void ExpressionStackPushDereference(ParseState *Parser, struct ExpressionStack **StackTop, Value *DereferenceValue);
static void ExpressionPushInt(ParseState *Parser, struct ExpressionStack **StackTop, long IntValue);
static void ExpressionPushFP(ParseState *Parser, struct ExpressionStack **StackTop, double FPValue);
...
static void ExpressionQuestionMarkOperator(ParseState *Parser, struct ExpressionStack **StackTop, Value *BottomValue, Value *TopValue);
static void ExpressionColonOperator(ParseState *Parser, struct ExpressionStack **StackTop, Value *BottomValue, Value *TopValue);
static void ExpressionPrefixOperator(ParseState *Parser, struct ExpressionStack **StackTop, LexToken Op, Value *TopValue);
static void ExpressionPostfixOperator(ParseState *Parser, struct ExpressionStack **StackTop, LexToken Op, Value *TopValue);
static void ExpressionInfixOperator(ParseState *Parser, struct ExpressionStack **StackTop, LexToken Op, Value *BottomValue, Value *TopValue);
static void ExpressionStackCollapse(ParseState *Parser, struct ExpressionStack **StackTop, int Precedence, int *IgnorePrecedence);
static void ExpressionStackPushOperator(ParseState *Parser, struct ExpressionStack **StackTop, enum OperatorOrder Order, LexToken Token, int Precedence);
static void ExpressionParseMacroCall(ParseState *Parser, struct ExpressionStack **StackTop, const char *MacroName, MacroDef *MDef);
static void ExpressionParseFunctionCall(ParseState *Parser, struct ExpressionStack **StackTop, const char *FuncName, int RunIt);


#ifdef DEBUG_EXPRESSIONS
/* show the contents of the expression stack */
void ExpressionStackShow(Picoc *picoc, struct ExpressionStack *StackTop)
{
    printf("Expression stack [0x%lx,0x%lx]: ", (long)picoc->HeapStackTop, (long)StackTop);
    ...
    while (StackTop != NULL) {
        if (StackTop->Order == OrderNone) {
            ...
            case TypeEnum:
                ...
                break;
            case Type_Type:
                PrintType(StackTop->Val->Val->Typ, picoc->CStdOut);
                printf(":type");
                break;
            default:
                printf("unknown");
    ...
    Value * LexValue)
{
    ...
    if (t == TokenIdentifier) {
        ...
        Value * VarValue;
        if (VariableDefined(Parser->picoc, LexValue->Val->Pointer)) {
            VariableGet(Parser->picoc, Parser, LexValue->Val->Pointer, &VarValue);
            if (VarValue->Typ == &Parser->picoc->TypeType)
                return 1;
        }
    }
...
void ExpressionStackPushValueNode(ParseState *Parser,
    struct ExpressionStack **StackTop, Value *ValueLoc)
{
    struct ExpressionStack *StackNode = VariableAlloc(Parser->picoc, Parser,
...
#endif
#ifdef DEBUG_EXPRESSIONS
    ExpressionStackShow(Parser->picoc, *StackTop);
#endif
}

/* push a blank value on to the expression stack by type */
Value *ExpressionStackPushValueByType(ParseState *Parser,
    struct ExpressionStack **StackTop, ValueType *PushType)
{
    Value *ValueLoc = VariableAllocValueFromType(Parser->picoc, Parser,
...
void ExpressionStackPushValue(ParseState *Parser,
    struct ExpressionStack **StackTop, Value *PushValue)
{
    Value *ValueLoc = VariableAllocValueAndCopy(Parser->picoc, Parser,
        PushValue, false);
    ExpressionStackPushValueNode(Parser, StackTop, ValueLoc);
}

void ExpressionStackPushLValue(ParseState *Parser,
    struct ExpressionStack **StackTop, Value *PushValue, int Offset)
{
...
void ExpressionStackPushDereference(ParseState *Parser,
    struct ExpressionStack **StackTop, Value *DereferenceValue)
{
    int Offset;
    int DerefIsLValue;
    Value *DerefVal;
...
{
    Value *ValueLoc = VariableAllocValueFromType(Parser->picoc, Parser,
                            &Parser->picoc->IntType, false, NULL, false);
...
{
    Value *ValueLoc = VariableAllocValueFromType(Parser->picoc, Parser,
                                 &Parser->picoc->FPType, false, NULL, false);
    ...
    int AllowPointerCoercion)
{
    ...
    if (FromValue->Typ == ToValue->Typ ||
            FromValue->Typ == Parser->picoc->VoidPtrType ||
            (ToValue->Typ == Parser->picoc->VoidPtrType &&
...
#endif
                DestValue->Typ = TypeGetMatching(Parser->picoc, Parser,
...
void ExpressionQuestionMarkOperator(ParseState *Parser,
    struct ExpressionStack **StackTop, Value *BottomValue,
    Value *TopValue)
{
    ...
    if (ExpressionCoerceInteger(TopValue)) {
        ...
        ExpressionStackPushValueByType(Parser, StackTop, &Parser->picoc->VoidType);
    }
}
...
void ExpressionPrefixOperator(ParseState *Parser,
    struct ExpressionStack **StackTop, LexToken Op, Value *TopValue)
{
    Value *Result;
    AnyValue *ValPtr;
    ValueType *Typ;
    ...
    case TokenAmpersand:
        ...
        ValPtr = TopValue->Val;
        Result = VariableAllocValueFromType(Parser->picoc, Parser,
                    TypeGetMatching(Parser->picoc, Parser, TopValue->Typ,
                        TypePointer, 0, Parser->picoc->StrEmpty, true),
                    false, NULL, false);
    ...
    case TokenSizeof:
        ...
        if (TopValue->Typ == &Parser->picoc->TypeType)
    ...
    default:
        ...
        if (TopValue->Typ == &Parser->picoc->FPType) {
...
void ExpressionInfixOperator(ParseState *Parser,
    struct ExpressionStack **StackTop, LexToken Op,
    Value *BottomValue, Value *TopValue)
{
    ...
    if (Op == TokenLeftSquareBracket) {
    ...
    else if ((TopValue->Typ == &Parser->picoc->FPType &&
                 BottomValue->Typ == &Parser->picoc->FPType) ||
              (TopValue->Typ == &Parser->picoc->FPType
        ...
        double ResultFP = 0.0;
        double TopFP = (TopValue->Typ == &Parser->picoc->FPType) ?
            TopValue->Val->FP : (double)ExpressionCoerceInteger(TopValue);
        double BottomFP = (BottomValue->Typ == &Parser->picoc->FPType) ?
        ...
        if (Op == TokenEqual || Op == TokenNotEqual) {
            ...
            HeapUnpopStack(Parser->picoc, sizeof(Value));
    ...
    while (TopStackNode != NULL && TopStackNode->Next != NULL &&
        ...
        if (FoundPrecedence >= Precedence && TopOperatorNode != NULL) {
            ...
            case OrderPrefix:
                ...
                HeapPopStack(Parser->picoc, NULL,
                    sizeof(struct ExpressionStack) +
                    sizeof(Value) +
                    TypeStackSizeValue(TopValue));
                HeapPopStack(Parser->picoc, TopOperatorNode,
                    sizeof(struct ExpressionStack));
            ...
            case OrderPostfix:
                ...
                /* pop the postfix operator and then the value - assume
                    they'll still be there until we're done */
                HeapPopStack(Parser->picoc, NULL, sizeof(struct ExpressionStack));
                HeapPopStack(Parser->picoc, TopValue,
                    sizeof(struct ExpressionStack) +
                    sizeof(Value) +
                    TypeStackSizeValue(TopValue));
            ...
            case OrderInfix:
                ...
                if (TopValue != NULL) {
                    ...
                    HeapPopStack(Parser->picoc, NULL,
                        sizeof(struct ExpressionStack));
                    HeapPopStack(Parser->picoc, BottomValue,
                        sizeof(struct ExpressionStack) +
                        sizeof(Value) +
...
void ExpressionGetStructElement(ParseState *Parser,
    struct ExpressionStack **StackTop, LexToken Token)
{
    Value *Ident;
    ...
    if (Parser->Mode == RunModeRun) {
        ...
        HeapPopStack(Parser->picoc, ParamVal,
            sizeof(struct ExpressionStack) +
            sizeof(Value) +
...
int ExpressionParse(ParseState *Parser, Value **Result)
{
        ...
        if ((((int)Token > TokenComma && (int)Token <= (int)TokenOpenBracket) ||
                ...
                if (Token == TokenOpenBracket) {
                    ...
                    if (IsTypeToken(Parser, BracketToken, LexValue) &&
                        ...
                        ExpressionStackCollapse(Parser, &StackTop, Precedence+1,
                            &IgnorePrecedence);
                        CastTypeValue = VariableAllocValueFromType(Parser->picoc,
                            Parser, &Parser->picoc->TypeType, false, NULL, false);
            ...
            if (LexGetToken(Parser, NULL, false) == TokenOpenBracket) {
                ...
                if (Parser->Mode == RunModeRun /* && Precedence < IgnorePrecedence */) {
                    ...
                    VariableGet(Parser->picoc, Parser, LexValue->Val->Identifier,
                        &VariableValue);
                    ...
                    } else if (VariableValue->Typ == &Parser->picoc->VoidType)
            ...
            TypeValue = VariableAllocValueFromType(Parser->picoc, Parser,
                &Parser->picoc->TypeType, false, NULL, false);
    ...
    if (StackTop != NULL) {
        ...
        if (Parser->Mode == RunModeRun) {
            ...
            *Result = StackTop->Val;
            HeapPopStack(Parser->picoc, StackTop, sizeof(struct ExpressionStack));
        } else
            HeapPopStack(Parser->picoc, StackTop->Val,
                sizeof(struct ExpressionStack) +
                sizeof(Value) +
    ...
    MacroDef *MDef)
{
    ...
    if (Parser->Mode == RunModeRun) {
        ...
        ExpressionStackPushValueByType(Parser, StackTop, &Parser->picoc->FPType);
        ReturnValue = (*StackTop)->Val;
        HeapPushStackFrame(Parser->picoc);
        ParamArray = HeapAllocStack(Parser->picoc,
    ...
    if (Parser->Mode == RunModeRun) {
        ...
        Parser->picoc->TopStackFrame->NumParams = ArgCount;
        Parser->picoc->TopStackFrame->ReturnValue = ReturnValue;
        for (Count = 0; Count < MDef->NumParams; Count++)
            VariableDefine(Parser->picoc, Parser, MDef->ParamName[Count],
    ...
    struct ExpressionStack **StackTop, const char *FuncName, int RunIt)
{
    ...
    if (RunIt) {
        ...
        VariableGet(Parser->picoc, Parser, FuncName, &FuncValue);
    ...
    do {
        if (RunIt && ArgCount < FuncValue->Val->FuncDef.NumParams)
            ParamArray[ArgCount] = VariableAllocValueFromType(Parser->picoc, Parser,
    ...
    if (RunIt) {
        ...
        if (FuncValue->Val->FuncDef.Intrinsic == NULL) {
            ...
            for (Count = 0; Count < FuncValue->Val->FuncDef.NumParams; Count++)
                VariableDefine(Parser->picoc, Parser,
            ...
            if (RunIt) {
                if (FuncParser.Mode == RunModeRun &&
                        FuncValue->Val->FuncDef.ReturnType != &Parser->picoc->VoidType)
